 ...
 *
 * The rules live in `core/inspection-core.js` and are deliberately plain
 * JavaScript: the same file runs on this server and in any front end, so scoring
 * can never drift between them. This module only puts a TypeScript face on it —
 * do not reimplement a rule here. See ../../README.md for what each rule does.
 */
import rawCore from "@core/inspection-core.js";

export type Size = "small" | "medium" | "large" | "";
export type CoreQuestionType = "rating" | "yesno" | "scale" | "number" | "choice";
export type CoreBucket = "well" | "improve" | "obs" | "skip";
...
interface Core {
  ...
  bucketOf(item: CoreItem, size?: Size): CoreBucket;
